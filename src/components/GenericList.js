    ...
    return (
        ...
        <div className="container top-0 flex-col items-center px-10 py-16 mx-auto text-black md:h-screen md:py-32 md:flex-row">
            ...
            {listItems.map((listItem) => (
                ...
                <div className="flex flex-row items-center h-full p-4 rounded bg-oat-dark dark:bg-gray-800">
                    {listItem.iconName ? <ListItemIcon icon={listItem.iconName} /> : <ListItemIcon icon={listIconName}/>}
                    {displayLink ? <a href={listItem.url}>{listItem.name}</a> : <span className="font-medium dark:text-white title-font">{listItem.name}</span>}
                </div>
                </div>
            ))}
            </div>
